...
export interface AgentCoreStackProps extends StackProps {
  ...
  /**
   * DynamoDB table name the agent runtime needs access to. The table's ARN is
   * derived from this stack's own account/region — no need to pass it separately.
   */
...
export class AgentCoreStack extends Stack {
  ...
  constructor(scope: Construct, id: string, props: AgentCoreStackProps) {
    ...
    const dynamoTableArn = this.formatArn({
      service: 'dynamodb',
      resource: 'table',
      resourceName: dynamoTableName,
    });
